...
--->
		<cfquery datasource="#This.DataSourceName#" name="qryRecords">						<!--- RAB 08/09/2012 --->
        SELECT	#This.ViewName#.*
        FROM    #This.ViewName# 
        		INNER JOIN dbo.ICITEM ON #This.ViewName#.#This.ITEMNOKey# = dbo.ICITEM.ITEMNO
...
--->
		...
		WHERE 	#This.PrimaryKey# = '#Arguments.RecordID#' 
				...
				AND (dbo.ICITEMO.OPTFIELD = 'SERIALNUM') AND (dbo.ICITEMO.VALUE = 'Y') AND	<!--- RAB 08/09/2012 --->
...
--->
			<cfquery datasource="#CURRENT_Datasource#" name="qryItem">						<!--- RAB 08/09/2012 --->
			SELECT 	dbo.ICITEM.ITEMNO
			FROM 	dbo.ICITEM
			...
			WHERE	dbo.ICITEM.ITEMNO = '#Arguments.ITEMNO#' AND
					...
					(dbo.ICITEMO.OPTFIELD = 'SERIALNUM') AND (dbo.ICITEMO.VALUE = 'Y')		<!--- RAB 08/09/2012 --->
			</cfquery>
	...
	<cfargument name="ItemNo" type="string" required="Yes">
        ...
        <!--- RAB 10/26/2012 Update.  With the new version of ACCPAC, we're going to grab cost from Recent Cost.  
			  Apparantly extra costs (like shipping) are being prorated and included in Recent Cost --->
...
<!---
qryRecords:<cfdump var="#qryRecords#"><br>
<cfabort>
--->		
...
<!---
LabelFile:<cfdump var="#LabelFile#"><br>
<cfabort>			
--->
...
<!---			<cfsavecontent variable="FieldData">"#strRecord.SerialNumber#"#chr(13)##chr(10)#</cfsavecontent>	---><!---RAB--->
	...
	<cfargument name="RecordID" type="string" required="Yes">
		<cfset local.FTPSuccess = 1>
		<cftry>
			<cffile action="write" file="//SCAN1-WIN7/LabelMatrixText/labels.txt" output="#Arguments.LabelFile#" nameconflict="OVERWRITE">
			<cfcatch type="any">
				<cfset local.FTPSuccess = 0>
			</cfcatch>
		</cftry>
		<!---
...
Arguments.ConsecutiveQuantity:<cfdump var="#Arguments.ConsecutiveQuantity#"><br>
--->
...
LastFourCharacters:<cfdump var="#LastFourCharacters#"><br>
FirstPartOfString:<cfdump var="#FirstPartOfString#"><br>
--->
...
<!---
StrippedFieldValue:<cfdump var="#StrippedFieldValue#"><br>
--->
						<!--- RAB 12/31/12 --->
						<cfset StrippedFieldValue = numberFormat(StrippedFieldValue + 1, ".")>
                        <cfset StrippedFieldValue = replace(StrippedFieldValue, ".", "")>
<!---
StrippedFieldValue:<cfdump var="#StrippedFieldValue#"><br>
--->
                        <cfset SerialNumberValue = LeadingZeros & StrippedFieldValue>
<!---
SerialNumberValue:<cfdump var="#SerialNumberValue#"><br>
--->
...
--->
					<cfset structInsert(strAdminItem, "ITEMDESC", getItemDescription(ITEMNO), True)>
