<cfcomponent extends="admin.assets.cfcs.Component">
    ...
    <!------------------------------------------------------------------------------------------------------------------------->
	<cffunction name="isAssigned" access="public" returntype="boolean" output="No">
	<cfargument name="ClassificationID" type="string" required="Yes">
		<cfset var ClassificationIsAssigned = 0>
		<cfset var qryConfigSystems = queryNew("")>

        <cfquery name="qryConfigSystems" datasource="#This.DataSourceName#">
        SELECT	ConfigSystemID
        FROM	tblConfigSystems 
        WHERE	ClassificationID = '#Arguments.ClassificationID#'
        </cfquery>
        <cfif qryConfigSystems.RecordCount NEQ 0>
			<cfset ClassificationIsAssigned = 1>
        </cfif>

		<cfreturn ClassificationIsAssigned>
	</cffunction>


    <!------------------------------------------------------------------------------------------------------------------------->
	<cffunction name="deleteClassification" access="public" output="No">
	<cfargument name="ClassificationID" type="string" required="Yes">
        ...
        <cfquery datasource="#This.DataSourceName#">
        DELETE FROM   tblClassificationAliases
        WHERE	ClassificationID = '#Arguments.ClassificationID#'
        </cfquery>
