...
--->		
	...
	<!-------------------------------------------------------------------------------------------------------------------------->
	<cffunction name="validate_importComponents" access="public" returntype="struct" output="No">
	<cfargument name="Record" type="struct" required="Yes">
		<cfset var stErrors = structNew()>
		<cfif validateRequired(Arguments.Record.ITEMNO) EQ 0>
			<cfset structInsert(stErrors, "ITEMNO", "Please enter an item number before clicking 'Continue'.", True)>
		</cfif>
		<cfreturn stErrors>
	</cffunction>
	...
	<cfargument name="ITEMNO" type="string" required="No">
		...
		<cfloop query="qryItems">
            ...
            <cfset CURRENT_DESCRIPTION = trim(qryItems.DESC)>
...
<!---						<cfset structInsert(strPriceListCategory, "MarkupPercent", CURRENTPriceListMarkUpPercent, True)>	--->
						...
						<cfset structInsert(strPriceListComponent, "Description", CURRENT_DESCRIPTION, True)>
						...
						<!--- Make the component active if a selling price was found --->
						<cfif NOT isMasterPriceList>
							<cfset strPriceListComponent = objPriceListComponents.getRecord(PriceListComponentID)>
							<cfif strPriceListComponent.SellPrice IS NOT "" AND 
								  isNumeric(strPriceListComponent.SellPrice) AND
								  strPriceListComponent.SellPrice GT 0>
...
--->
	...
	<!-------------------------------------------------------------------------------------------------------------------------->
	<cffunction name="webDisabledComponents" access="public" returntype="string" output="no">
	<cfargument name="ITEMNO" type="string" required="Yes">
		<cfset var Outcome = "">
		<cfset var qryItem = queryNew("")>

		<cfif trim(Arguments.ITEMNO) IS "">
			<cfset Outcome = "Item_Not_Entered">		
		<cfelse>
			...
			<cfif qryItem.RecordCount EQ 0>
				<cfset Outcome = "Item_Not_Found">		
			<cfelseif qryItem.ALLOWONWEB EQ 1>
				<cfset Outcome = "Item_is_web_enabled">		
			<cfelse>		
				<cfquery datasource="#This.DataSourceName#">
				DELETE FROM	tblPriceListComponents
...
<!---		<cfset structDelete(strPriceListCategoryNEW, "MarkupPercent")>	--->
			...
			<cfloop query="qryPriceListComponentsCOPY">
				...
				<cfset structInsert(strPriceListComponentNEW, "Description", qryPriceListComponentsCOPY.Description, True)>
...
<!---			<cfset structDelete(strPriceListComponentNEW, "FixedPrice")>	--->
	...
	<cfargument name="RecordID" type="string" required="Yes">
        ...
        <cfset objComponentPrices.deletePricesForPriceList(Arguments.RecordID)>        
		<cfreturn>
	</cffunction>
...
<!---<cfargument name="MarkupPercentage" type="string" required="no">--->
		...
		<cfset var strPriceList = structNew()>
        ...
        <cfset var CURRENTPriceListCategoryID = "">
								...
								<tr>
									<cfif NOT Arguments.ExportableConfigurator>
										<td width="50%" valign="top" align="center" style="font-size:12px; font-weight:bold;">
											<img src="https://test.nor-tech.com/images/logo.gif" alt="" name="logo" width="150" border="0"><br>
											www.nor-tech.com
										</td>
									</cfif>
	...
	<!---									(877) 808-1010	--->
					...
					<cfloop query="qryPriceListCategories">
						...
						<cfif qryPriceListComponents.RecordCount GT 0>
							...
							<cfloop query="qryPriceListComponents">
                            	<!--- RAB 09/05/2012, commenting out this condition --->
<!---							<cfif NOT objPartsAdmin.isGarageSaleItem(qryPriceListComponents.ITEMNO)>	--->
									...
									<tr<cfif qryPriceListComponents.CurrentRow mod 2> style="background-color:##e5e5e6"</cfif>>
										...
										<!--- IF EXPORTABLE PRICE LIST --->
										<cfif Arguments.ExportableConfigurator>
                                        
                                        	<!--- Try to get the fixed selling price for this component --->
											<cfset SellingPrice =  objCust.getCustomerFixedPrice(Arguments.CustID, qryPriceListComponents.PriceListComponentID)>
                                            
                                            <!--- Try to get the markup percentage for this category --->
                                            <cfif SellingPrice IS "">
                                            	<cfset MarkupPercent = objCust.getCustomerMarkupPercent(Arguments.CustID, CURRENTPriceListCategoryID)>
...
<!---										
										<cfif Arguments.ExportableConfigurator AND 
											...
											<cfif strLogin.MarkupType IS "Margin">
												<cfset SellingPrice = ceiling(SellingPrice / (1 - strLogin.PercentComponents))>
                                            <cfelse>
