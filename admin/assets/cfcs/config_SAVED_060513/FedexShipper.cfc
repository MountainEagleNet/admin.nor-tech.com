...
<!---
	...
	Your user ID is: robpanico
	
	Test Account information
	Test Password:T8f0qIYIxDye9XPNuLcETHOkD
	Test Account Number:510087402 (for FedEx Web Services for Shipping only)
	Test Meter Number:118571691 (for FedEx Web Services for Shipping only)
	...
	Note: The FedEx Freight LTL accounts provided above are for testing only. Use your FedEx Freight LTL Shipper and/or Bill To 
	account number provided by FedEx Freight when you move to Production.
	...
	If you need a reminder of your Developer Test Key, log in to the FedEx Developer Resource Center, select the Develop and Test 
	Your Application link and then click on the Obtain Devloper Test Key/Access test environment button to request your test key.

	Test instructions:
	Check the FedEx Developer Resource Center Technical Resources Announcements to coordinate test time (that is, to avoid planned outages). 
	Download testing WSDLs, sample code and documentation from the Get Started section.

	Fedex Web Services for Office and Print
	By default, your Developer Test Key is disabled for FedEx Web Services for Office and Print. Please refer to the Develop and Test Your 
	Application page within the Developer Resource Center for the procedures required to enable your Developer Test Key.

	You're almost ready to launch
	After you've successfully tested your application and are ready to go into production, go to the FedEx Developer Resource Center to 
	obtain your Production key. If you need more assistance, we're here to help. Select the Support link from the FedEx Developer Resource 
	Center to view a list of support options specific to your location.
--->
	<cfset variables.accounts = structNew()>
	<cfset variables.accounts.test = structNew()>
	<cfset variables.accounts.test.key = "">
	<cfset variables.accounts.test.password = "T8f0qIYIxDye9XPNuLcETHOkD">
	<cfset variables.accounts.test.accountNo = "510087402">
	<cfset variables.accounts.test.meterNo = "118571691">
	<cfset variables.accounts.test.shipperAddress1 = "901 East Cliff Road">
	<cfset variables.accounts.test.shipperCity = "Burnsville">
	<cfset variables.accounts.test.shipperState = "MN">
	<cfset variables.accounts.test.shipperZip = "55337">
	<cfset variables.accounts.test.shipperCountry = "US">
	<cfset variables.accounts.test.fedexUrl = "https://wsbeta.fedex.com:443/web-services">
		
	<cfset variables.accounts.live = structNew()>
	<cfset variables.accounts.live.key = "3pTFSjnrG1CWjYyo">
	<cfset variables.accounts.live.password = "wLD5AL7Oe2owXE7uYgh6xobJn">
	<cfset variables.accounts.live.accountNo = "228722456">
	<cfset variables.accounts.live.meterNo = "104610828">
	<cfset variables.accounts.live.shipperAddress1 = "901 East Cliff Road">
	<cfset variables.accounts.live.shipperCity = "Burnsville">
	<cfset variables.accounts.live.shipperState = "MN">
	<cfset variables.accounts.live.shipperZip = "55337">
	<cfset variables.accounts.live.shipperCountry = "US">
	<cfset variables.accounts.live.fedexUrl = "https://ws.fedex.com/web-services">
	
	<cfset variables.mappings = structNew()>
	<cfset variables.mappings.FFovernight = "FIRST_OVERNIGHT">
	<cfset variables.mappings.FPovernight = "PRIORITY_OVERNIGHT">
	<cfset variables.mappings.FSovernight = "FEDEX_2_DAY">
	<cfset variables.mappings.FGround = "FEDEX_GROUND">
	
	<cffunction name="init" access="public" returntype="any" output="false">
	<cfargument name="mode" type="string" required="yes">
		<cfif arguments.mode is "live">
			<cfset variables.key = variables.accounts.live.key>
			<cfset variables.password = variables.accounts.live.password>
			<cfset variables.accountNo = variables.accounts.live.accountNo>
			<cfset variables.meterNo = variables.accounts.live.meterNo>
			<cfset variables.fedexUrl = variables.accounts.live.fedexUrl>
			<cfset variables.shipperAddress1 = variables.accounts.live.shipperAddress1>
			<cfset variables.shipperCity = variables.accounts.live.shipperCity>
			<cfset variables.shipperState = variables.accounts.live.shipperState>
			<cfset variables.shipperZip = variables.accounts.live.shipperZip>
			<cfset variables.shipperCountry = variables.accounts.live.shipperCountry>
		<cfelse>
			<cfset variables.key = variables.accounts.test.key>
			<cfset variables.password = variables.accounts.test.password>
			<cfset variables.accountNo = variables.accounts.test.accountNo>
			<cfset variables.meterNo = variables.accounts.test.meterNo>
			<cfset variables.fedexUrl = variables.accounts.test.fedexUrl>
			<cfset variables.shipperAddress1 = variables.accounts.test.shipperAddress1>
			<cfset variables.shipperCity = variables.accounts.test.shipperCity>
			<cfset variables.shipperState = variables.accounts.test.shipperState>
			<cfset variables.shipperZip = variables.accounts.test.shipperZip>
			<cfset variables.shipperCountry = variables.accounts.test.shipperCountry>
		</cfif>
		<cfreturn this />
	</cffunction>
	...
	<cfargument name="record" type="struct" required="yes">
		<cfset stRates = fetchRates(
							shipperAddress1 = variables.shipperAddress1,
							shipperCity = variables.shipperCity,
							shipperState = variables.shipperState,
							shipperZip = variables.shipperZip,
							shipperCountry = variables.shipperCountry,
							shipToAddress1 = "",
							shipToCity = "",
							shipToState = arguments.record.to.State,
							shipToZip = arguments.record.to.Zip,
							shipToCountry = "US",
							pkgWeight = arguments.record.weight,
							shipToResidential = arguments.record.to.residence)>
		<cfif stRates.Success is "NO">
			<cfset arguments.record.result.success = 0>
			<cfset arguments.record.result.errormessage = stRates.Response[1].Msg>
			<cfset arguments.record.result.EstimatedFreight = 0>
		<cfelseif stRates.Success is "YES">
			<cfset NumRates = arrayLen(stRates.Rates)>
			<cfset RateKey = variables.mappings[arguments.record.params.ShippingMethod]>
			<cfloop from="1" to="#NumRates#" step="1" index="r">
				<cfset stRate = stRates.Rates[r]>
				<cfif stRate.Type is RateKey>
					<cfset arguments.record.result.EstimatedFreight = (stRate.Total - stRate.Discount)>
					<cfbreak>
				</cfif>
			</cfloop>
			<cfset arguments.record.result.success = 1>
		</cfif>	
		<cfreturn arguments.record.result>
	</cffunction>
	
	<cffunction name="fetchRates" access="public" returntype="struct" output="true">
				...
				<ns:RequestedShipment>
					...
					<ns:Shipper>
						...
						<cfif len(trim(arguments.shipperUrbCode))>
							...
							<ns:CountryCode>#arguments.shipperCountry#</ns:CountryCode>
							<ns:Residential>#arguments.shipperIsResidential#</ns:Residential>
						</ns:Address>
					</ns:Shipper>
