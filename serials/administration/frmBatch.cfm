...
<!---
	...
	Template:		frmBatch.cfm
	Task:			serials_admin_batch_new
--->
...
<!---		<cfset qryItems = objICITEM.searchRecords(SearchRecord, "query", "ITEMNO", 0)>	--->
			<cfset qryItems = objScannerBatchItems.searchAdminItems(SearchRecord)>
...
<form name="ItemNumberSearch" action="index.cfm?task=serials_admin_batch_new" method="post">
	<tr>
			...
			<font color="FF0000">
				...
				<cfelseif Error IS "No Match Found">
					...
					<cfelseif trim(FORM.ITEMNO) IS NOT "">
						Item Number '#FORM.ITEMNO#' was either not found, or is already included in the list of Batch Number Items.
...
<cfif ExecuteSearch AND isDefined("qryItems") AND Error IS "">
	...
	<td valign="top" class="textmain" colspan="3">
		...
		<form name="PickItems" action="index.cfm?task=serials_admin_batch_save" method="post">

		<tr>
			<td valign="top" class="textmain" colspan="2">
				Check the boxes for all items that you want to add to<br>the master list of "Batch Number Items", then click "Add".
			</td>
...
<!---
					<a href="index.cfm?task=serials_admin_batch_save&ITEMNO=#urlEncodedFormat(qryItems.ITEMNO)#&ITEMDESC=#urlEncodedFormat(qryItems.DESC)#">
						[add]
					</a>
--->
				</td>
