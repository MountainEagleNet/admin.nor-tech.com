...
<!---
	...
	Template:		frmBatch2.cfm
	Task:			serials_admin_batch2_new
--->
...
<!---		<cfset qryItems = objICITEM.searchRecords(SearchRecord, "query", "ITEMNO", 0)>	--->
			<cfset qryItems = objSerialBatch2.searchAdminItems(SearchRecord)>
...
<form name="ItemNumberSearch" action="index.cfm?task=serials_admin_batch2_new" method="post">
	<tr>
...
<cfif ExecuteSearch AND isDefined("qryItems") AND Error IS "">
	...
	<td valign="top" class="textmain" colspan="3">
		...
		<form name="PickItems" action="index.cfm?task=serials_admin_batch2_save" method="post">

		<tr>
			<td valign="top" class="textmain" colspan="2">
				Check the boxes for all items that you want to add to<br>the master list of "Batch 2 Items", then click "Add".
			</td>
...
<!---
					<a href="index.cfm?task=serials_admin_batch2_save&ITEMNO=#urlEncodedFormat(qryItems.ITEMNO)#&ITEMDESC=#urlEncodedFormat(qryItems.DESC)#">
						[add]
					</a>
--->
				</td>
