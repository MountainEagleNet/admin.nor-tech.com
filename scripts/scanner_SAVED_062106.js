Scanner = function() {
	...
	this.showPopup = function(fieldname, content) {
		...
		pw.style.width = "225px";
		pw.style.height = "125px";
	...
	this.activateFields = function(fieldname) {
		...
		var maxfields = self.countFields() + 1;
	...
	this.isLastField = function(fieldname) { 
		var lastfield = false;
		if(self.getFieldNumber(fieldname) == self.countFields()) { lastfield = true; }
		return lastfield;
	};
	
	this.isFirstField = function(fieldname) { 
		var firstfield = false;
		if(self.getFieldNumber(fieldname) == 1) { firstfield = true; }
		return firstfield;
	};
	...
	this.nextField = function(fieldname) { 
		var maxnum = Number(self.countFields());
	...
	this.isValidMask = function(fieldname) { 
		...
		if(self.isFirstField(fieldname) == true) {
			...
			if(secmask != self.getMaskTemplate()) { valid = false; }
		}
		return valid;
	};
	...
	this.init = function() {
		...
		self.settings.styles.duplicate.border = "1px solid gold";
		self.settings.styles.duplicate.backgroundColor = "white";
		self.settings.styles.duplicate.color = "gold";
	...
	this.checkMaskTemplate = function() {
		var firstname = self.getFieldName(1);
	...
	this.captureKey = function() {
		var allowkey = true;
		if(self.getKeyCode() == event.keyCode) {
			if(self.isEmpty(event.srcElement.name) == true && self.getEmptyAlert() == true) { 
				self.doEmptyError(event.srcElement.name); 
			}
			else if(self.isValidMask(event.srcElement.name) == false && self.getMaskAlert() == true) { 
